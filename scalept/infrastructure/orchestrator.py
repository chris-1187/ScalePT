...
class ClusterOrchestrator:
    ...
    def __init__(self, num_nodes: int = None):
        ...
        if num_nodes is not None:
            max_nodes = len(self.cfg.worker_nodes)
            if num_nodes < 1 or num_nodes > max_nodes:
                raise ValueError(f"num_nodes must be between 1 and {max_nodes}")

            self.cfg.worker_nodes = self.cfg.worker_nodes[:num_nodes]
            self.cfg.args.nnodes_used = num_nodes
    ...
    def run_distributed_training(self,
                                 ...
                                 custom_tag: str = ""):
        """
        Launches torchrun on all worker nodes in the cluster.
        """
        ...
        # Setup training cmd for each worker
    ...
    def run_evaluation(self,
                       ...
                       total_frames=None):
        """
        Runs inference/evaluation on all worker nodes in the cluster.
        """
        import time
        ...
        if id is not None:
            inference_id = f"eval_{sampling_strategy}_{fusion_strategy}_{id}"
        else:
        ...
        # Redis queue
        total_available_frames = self.get_total_frames(sequence=sequence)
        frames_to_queue = total_available_frames
        if total_frames is not None:
            frames_to_queue = min(total_available_frames, total_frames)

        redis_host = self.cfg.primary_node.ip

        run_timestamp = int(time.time())
        queue_name = f"spt_queue_{sampling_strategy}_{fusion_strategy}_{frames_to_queue}_{run_timestamp}"

        console.print(f"[dim]Connecting to Redis at {redis_host}:6379...[/dim]")
        try:
            ...
            # push all frame indices to the queue list
            frame_indices = list(range(frames_to_queue))
            r.rpush(queue_name, *frame_indices)
            r.expire(queue_name, 86400)
            console.print(f"[bold green]✓ Pushed {frames_to_queue} frames to Redis queue '{queue_name}'[/bold green]")
        except redis.ConnectionError:
            console.print(f"[bold red]CRITICAL: Could not connect to Redis server at {redis_host}:6379.[/bold red]")
            return
        ...
        active_nodes = list(self.node_config.keys())

        # Setup inference cmd for each worker
        for node_name in active_nodes:
            ip = self.node_config[node_name]['ip']
            console.print(f"[green]Spawning worker on {node_name} ({ip})[/green]")

            cmd = inference_cmd.format(node_name=node_name)
            ...
            conn.run("pkill -f 'spt_worker.inference'", warn=True, hide=True)
            conn.run("pkill -f 'spt_worker.eval'", warn=True, hide=True)

            conn.run(cmd, hide=True)

        self.aggregate_metrics(remote_exp_dir, inference_id, active_nodes, frames_to_queue, cluster_start_time)
    ...
    def aggregate_metrics(self, remote_exp_dir, inference_id, node_names, total_frames, cluster_start_time):
        ...
        for node_name in node_names:
            ...
            if self.primary.run(f"[ -f {file_path} ]", warn=True, hide=True).failed:
                continue
            ...
            total_tiles += data["raw_metrics"].get("total_tiles", 0)
        ...
        avg_tiles_per_frame = total_tiles / total_frames_processed if total_frames_processed > 0 else 0.0
        ...
        final_metrics = {
            ...
            "results": {
                ...
                "cluster_throughput_fps": round(total_frames_processed / total_cluster_time_sec, 2) if total_cluster_time_sec > 0 else 0.0,
                ...
                "average_tiles_per_frame": round(avg_tiles_per_frame, 2),
